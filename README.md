...
这个项目帮你**每天自动点续期**，让你挂在上面的 Bot / 网站永不停止，完全免费、不用自己每天登录去点。
...
脚本每天定时跑一次，把账号下所有 Bot / 网站全部续期，然后通过 `notify.py`（Telegram + SMTP 双通道）通知你结果。
...
默认每天**北京时间 09:00** 跑一次。要改，编辑 `.github/workflows/renew.yml` 里的 `cron`（注意 GitHub 用 UTC 时间，北京时间减 8 小时）：

```
'0 1 * * *'   # UTC 01:00 = 北京时间 09:00，每天
```
