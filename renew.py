...
# -*- coding: utf-8 -*-
"""
MWS (cloud.puratya.com) 自动续期脚本 (UI点击 + API续期 + 离线自动唤醒 + TG截图 + 强力清理版)
"""
...
def main():
    ...
    # 清理旧截图并确保目录存在
    clean_temp_files(SCREENSHOT_DIR)
    ensure_dir(SCREENSHOT_DIR)
    
    with sync_playwright() as p:
        browser = None
        try:
            # 【核心安全修改】：加入 try 块，确保发生异常也会走到 finally 关闭浏览器
            ...
            # 【核心安全修改】：无论成功与否，强制清理僵尸进程和缓存空间
            if browser:
                browser.close()
                print("\n[*] 已清理并释放 Playwright 浏览器进程。")
